...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (file) {
      ...
      const fileName = `personal-discovery-${session!.user!.id}-${Date.now()}-${file.name}`
    ...
    const personalDiscovery = await db.personalDiscovery.upsert({
      ...
      update: {
        ...
        opportunities: validatedData.opportunities.split(',').map(s => s.trim()),
        threats: validatedData.threats.split(',').map(s => s.trim()),
        achievements: validatedData.achievements.split(',').map(s => s.trim()),
        familyAspirations: [validatedData.familyAspirations],
        careerAspirations: [validatedData.careerAspirations],
        financialBusinessAspirations: [validatedData.financialBusinessAspirations],
        socialAspirations: [validatedData.socialAspirations],
        desiredPosition: validatedData.desiredPosition.split(',').map(s => s.trim()),
        requiredSkills: validatedData.requiredSkills.split(',').map(s => s.trim()),
        coursesAndTrainings: validatedData.coursesAndTrainings.split(',').map(s => s.trim()),
        strategies: validatedData.strategies.split(',').map(s => s.trim()),
        shortTermGoals: validatedData.shortTermGoals.split(',').map(s => s.trim()),
